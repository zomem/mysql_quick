...
/// 
/// 也可以通过 Vec<T> 的方式批量新增：
/// // 注意：此方法不适合字符串里面有 ," 这个连续出现的情况。如 "abc,"def"
/// 
...
/// }
/// let vec_data = vec![
///     Item {content: String::from("aaa"), total: 12, uid: 3, des: String::from("nn")},
///     Item {content: String::from("bb"), total: 1, uid: 3, des: String::from("aaa")},
/// ];
    ...
    ($t:expr, $v: expr) => {
        {
            ...
            value.pop();
            let sql: String = "INSERT INTO ".to_string() + $t + field_name.as_str()
                + " VALUES" + value.as_str();
